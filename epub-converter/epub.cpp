...
PackageFile::PackageFile(std::string id, std::string title,
      ...
      : XMLFile("EPUB/package.opf",
        ...
        XMLTag * nav_man = new ManifestItem(nav->m_name, "nav", "application/xhtml+xml");
...
int main() {
  ...
  Nav * nav = new Nav(toc, new XMLStringTag("h1", "spis treści"), new OrderedList({new ListItem(new Anchor("plik.xhtml", "pliczek"))}));
  XHTMLAdapter * nav_file = new XHTMLAdapter("nav.xhtml", {}, nav);
  PackageFile * package = new PackageFile("1", "tytuł", "autor", "pl", nav_file,
